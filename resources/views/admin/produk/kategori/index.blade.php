...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.produk.kategori.create') }}" class="px-4 sm:px-5 py-2.5 bg-sky-500 text-white rounded-xl font-medium hover:bg-sky-600 transition shadow-sm shadow-sky-200 flex items-center gap-2 text-sm sm:text-base">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="md:hidden space-y-3">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            ...
            <div class="flex items-center gap-3 mb-3 pb-3 border-b border-slate-100">
                @if($kategori->gambar)
                    <img src="{{ asset($kategori->gambar) }}" class="w-12 h-12 rounded-lg object-cover shrink-0" alt="">
                @else
                ...
                @endif
                <div class="min-w-0">
                    <div class="font-semibold text-slate-800 text-sm truncate">{{ $kategori->nama }}</div>
                    <div class="text-xs text-slate-400">{{ $kategori->slug }} &middot; {{ $kategori->pakets->count() }} paket</div>
                </div>
            </div>

            {{-- Detail --}}
            <div class="flex items-center justify-between mb-4">
                <div class="text-sm text-slate-500 pr-2">{{ Str::limit($kategori->deskripsi, 60) }}</div>
                <span class="shrink-0 px-2.5 py-1 rounded-lg text-xs font-medium bg-slate-100 text-slate-600">Urutan {{ $kategori->urutan }}</span>
            </div>

            {{-- Actions --}}
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.produk.kategori.edit', $kategori->id) }}" 
                ...
                <form action="{{ route('admin.produk.kategori.destroy', $kategori->id) }}" method="POST" onsubmit="return confirm('Yakin hapus kategori & semua paket di dalamnya?')" class="flex-1">
                    @csrf
                    @method('DELETE')
